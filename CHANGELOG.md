...
### Fixed
- **Webm race condition**: Intermediate file cleanup now scoped to current track only. Previously deleted all `.webm` files in the album directory, which with concurrency 2 would remove another track's in-progress conversion.
- **NAS ramdisk overflow**: All containers now have `json-file` logging with 5MB max-size cap. Gluetun restart policy changed from `unless-stopped` to `on-failure:3` to prevent infinite crash loops filling the 400MB QNAP root ramdisk.
- **Gluetun HTTP proxy**: Added missing `HTTPPROXY=on` env var to compose template. Without it, gluetun starts the VPN tunnel but never enables the HTTP proxy on port 8888.
- **stream_served spam**: Demoted activity log event from `info` to `debug` so playback doesn't flood the activity panel.
- **PII cleanup**: Removed hardcoded user names, private IPs, personal paths, deploy configs, and brainstorm transcripts. Repository now safe for public access.
- **Bootstrap `curl|bash`**: All `read` calls now use `/dev/tty` so interactive prompts work when piped via `curl -sL ... | bash`.
...
### Changed
- No more hardcoded users: seed users removed from DB seeding
