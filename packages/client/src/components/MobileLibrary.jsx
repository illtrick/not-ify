    ...
    <div style={{ padding: 12 }}>
      ...
      {recentlyPlayed.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: COLORS.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Recently Played</div>
          <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4, WebkitOverflowScrolling: 'touch' }}>
            {recentlyPlayed.slice(0, 8).map((r, i) => (
              <div key={`mrp-${i}`} style={{ flexShrink: 0, width: 120, cursor: 'pointer' }}
                onClick={() => {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {albums.length > 0 && (
            <span style={{ fontSize: 13, color: COLORS.textSecondary, background: COLORS.hover, borderRadius: 10, padding: '2px 8px' }}>{albums.length}</span>
          )}
        </div>
      ...
      {showLibraryFilter && (
        <input type="text" placeholder="Filter albums..." value={libraryFilter} onChange={e => setLibraryFilter(e.target.value)} autoFocus
          style={{ width: '100%', padding: '8px 12px', marginBottom: 10, background: COLORS.hover, border: `1px solid ${COLORS.border}`, borderRadius: 6, color: COLORS.textPrimary, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
      )}
      ...
      ) : albums.map(({ albumId, artist, album, tracks, coverArt, mbid, rgid }) => {
        const isPlaying_ = currentAlbumInfo?.artist === artist && currentAlbumInfo?.album === album;
        return (
          <div key={`${artist}::${album}`}
            onClick={() => { openAlbumFromLibrary(albumId, { artist, albumName: album, tracks, coverArt, mbid, rgid }); }}
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 13, color: COLORS.textSecondary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 2 }}>Album · {artist}</div>
            </div>
